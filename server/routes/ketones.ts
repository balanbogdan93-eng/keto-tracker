...
function checkKetoneBadges(level: number, stats: any) {
  ...
  if (level >= 0.5 && !badges.includes('keto_confirmed')) {
    badges.push('keto_confirmed'); newBadges.push('keto_confirmed');
  }
  if (level >= 1.5 && !badges.includes('deep_ketosis')) {
    badges.push('deep_ketosis'); newBadges.push('deep_ketosis');
  }
...
router.post('/', (req: Request, res: Response) => {
  const { date, time, level, glucose, notes } = req.body;
  ...
  const result = db.prepare(`
    INSERT INTO ketones (date, time, level, glucose, notes)
    VALUES (?, ?, ?, ?, ?)
  `).run(date, time, level, glucose ?? null, notes || null);

  const ketone = db.prepare('SELECT * FROM ketones WHERE id = ?').get(result.lastInsertRowid);

  const stats = getOrCreateStats();
  let xp_gained = 5;
  if (level >= 1.5 && level <= 3.0) xp_gained += 20;
...
router.delete('/:id', (req: Request, res: Response) => {
  db.prepare('DELETE FROM ketones WHERE id = ?').run(req.params.id);
